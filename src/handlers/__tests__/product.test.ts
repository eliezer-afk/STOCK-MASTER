...
beforeAll(() => {
    serverInstance = app.listen(4000); // Guarda la referencia al servidor HTTP
});

describe('POST/api/products',() => {
    it('should display validation errors',async() => {
        const response = await request(serverInstance).post('/api/products').send({})
...
describe('Get/api/products',() => {
    ...
    it('should check if api/products url exists', async() =>{
        const response = await request(serverInstance).get('/api/products')
        expect(response.status).not.toBe(404)
    })
...
describe('GET/api/products/:id',() =>{
    it('Should return a 404 respnse for a non-existent product',async() => {
        ...
        const response = await request(serverInstance).get(`/api/products/${productId}`)
    ...
    it('should check a valid ID in the URL', async() => {
        const response = await request(serverInstance).get('/api/products/not-valid-url')
        expect(response.status).toBe(400)
        expect(response.body).toHaveProperty('errors')
        expect(response.body.errors).toHaveLength(1)
    ...
    it('get a JSON response for a single product', async () => {
        const response = await request(serverInstance).get('/api/products/1')
        expect(response.status).toBe(400)
        expect(response.body).toHaveProperty('errors')
        expect(response.body.errors).toHaveLength(1)
        expect(response.body.errors[0].msg).toBe('ID no valido')
...
describe('PUT/api/product/:id', () => {
    ...
    it('should display validation error messages when updating a product',async() => {
        const response = await request(serverInstance).put('/api/product/1').send({})
    ...
    it('should return a 404 response for a non-existent propduct', async () => {
        const productId=2000
        const response = await request(serverInstance).put('/api/products').send({
            name: "Mouse",
            price: 300,
            availability:true
        })
    ...
    it('should update an existent propduct with valid data', async () => {
        const response = await request(serverInstance).put('/api/products/1').send({
            name: "Mouse",
            price: 300,
            availability: true
        })
...
describe('DELETE/api/product/:id',() => {
    it('should check a valid ID', async() => {
        const response = await request(serverInstance).delete('/api/products/not-valid')
        expect(response.status).toBe(400)
        expect(response.body).toHaveProperty('errors')
        expect(response.body.errors[0].msg).toBe("ID no valido")
    })
    ...
    it('should return 404 response for a non existent product',async() => {
        const productId=2000
        const response = await request(serverInstance).delete(`/api/products/${productId}`)
        expect(response.status).toBe(404)
        expect(response.body.error).toBe("Producto no encontrado")
    ...
    it('should delete a product',async() => {
        const response = await request(serverInstance).delete('/api/products/1')
        expect(response.status).toBe(200)
...
describe('PATCH /api/product/:id', () => {
    it('should return a 404 response for a non-existing product', async () => {
        const response = await request(serverInstance).get('/api/products/non-existing-id');
        expect(response.status).toBe(404);
        expect(response.body).not.toHaveProperty('data');
    });
